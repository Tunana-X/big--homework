...
import com.soft.common.util.PathUtil;
...
public class UploadAdminController {
	...
	static{
		...
		try {
			props.load(UploadAdminController.class.getClassLoader().getResourceAsStream(path));
		} catch (Exception e) {
			props = new Properties();
		}
	}
	...
	public String UploadImg(@RequestParam("upload") MultipartFile file,String num,
		...
		try {
			...
			String saveFile= PathUtil.getRootPath(savePath+"\\"+file_name);
			//文件类型限制
