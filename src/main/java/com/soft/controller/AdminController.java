...
public class AdminController {
	...
	public String getSalesChart(
		...
		try {
			// ✅ 仅在用户提交了搜索表单时校验空值
			if (goods_id != null || (goods_name != null && !goods_name.isEmpty())) {
				if (goods_id == null && (goods_name == null || goods_name.isEmpty())) {
					throw new IllegalArgumentException("必须输入商品ID或商品名称");
				}

				Date _startDate = DateUtil.parse(startDate);
				Date _endDate = DateUtil.parse(endDate);
				_startDate = (_startDate == null) ? DateUtil.addDays(new Date(), -30) : _startDate;
				_endDate = (_endDate == null) ? new Date() : _endDate;
				...
				List<Goods> goodss = adminManager.listSalesHistory(goods);
				List<OrdersDetail> salesHistory = new ArrayList<>();
				for (Goods g : goodss) {
					OrdersDetail detail = new OrdersDetail();
					detail.setSaleDate(DateUtil.parse(g.getGoods_date()));
					detail.setTotalSales(g.getSale_count());
					salesHistory.add(detail);
				}
				...
				List<Integer> predictions = salesPredictionService.predict(sales);

				model.addAttribute("salesHistory", salesHistory);
				model.addAttribute("predictions", predictions);
				model.addAttribute("goodsId", goods_id);
				model.addAttribute("goodsName", goods_name);
				...
				// ✅ 用户未提交搜索时，不进行任何查询
				model.addAttribute("startDate", DateUtil.getCurDate());
				model.addAttribute("endDate", DateUtil.getCurDate());
			}
		} catch (Exception e) {
